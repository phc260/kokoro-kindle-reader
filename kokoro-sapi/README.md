...
| `engine.rs` | `KokoroEngine` — `ISpTTSEngine` + `ISpObjectWithToken`; forwards `Speak` over the pipe and streams the returned sub-frames straight to the SAPI site in ~250 ms blocks (no buffering) with `SPVES_ABORT` checks, while reporting SAPI word/bookmark events (so Kindle's event-driven narrator advances past sentence one). `ChunkMap::offset_of` is where an event's character position becomes an audio offset - from a model-derived mark when the chunk carries one, by linear interpolation when it doesn't. |
