...
| Path | What |
|---|---|
| `kokoro-host/` | The windowless tray host (x64): `main.rs` (tao event loop + tray + `auto-launch` + Kindle-watcher tick), `pipe.rs` (named-pipe server; owns chunking + prefetch + pacing), `native_synth.rs` (serialized Rust synth, GPU or CPU EP + `controls.json` reader) + `text.rs`/`espeak.rs` (kokoro-js text normalizer + espeak-ng FFI), `split_text.rs` (the sentence-chunk splitter), `model_patch.rs` (the 273-byte in-memory ONNX graph edit that exposes the model's per-token durations), `ctx.rs` (`CoreCtx` — the transport-neutral synth context: paths, the one worker, the one `HostState`, `available_voices`; built once and cloned into the pipe's `KindleCtx` and the endpoint's `WebCtx`), `state.rs` (`HostState` — the general lock-free cell: the any-client audio clock, the bench slot), `kindle_state.rs` (`KindleState` — the Kindle half over it: reading belief, Kindle-only audio clock, live pause, pid; unreachable from the HTTP endpoint), `kindle_ctl.rs` (the Kindle-control thread: hands-free Ctrl+A toggle of Read Aloud + `WM_CLOSE`, via raw Win32/Toolhelp32 — UI Automation only to find Kindle's window and dismiss an open flyout, never to read the reader's state back), `kindle_watch.rs` (polls for Kindle, spawns the injector), `webserve.rs` (the browser path's loopback HTTP endpoint on 127.0.0.1:8787 and its four security checks — `/synth` calls `native_synth::synth` directly, `/ocr` drives the `kokoro-ocr` worker). `ctx.rs`/`state.rs`/`kindle_state.rs` (the context + state split). Builds for **two targets**: the pipe, Kindle and tray modules are `cfg(windows)`, and a Linux build is the shared core plus `webserve.rs`. `build.rs` branches on the target OS — it links the espeak-ng library and stages the runtime libraries + `espeak-ng-data` (with an `$ORIGIN` rpath on Linux). |
...
| `native-deps/` | Synth **dependency provisioning** only (no source): `fetch-deps.py` populates the gitignored dep folders alongside itself (`native-deps/windows/runtime/` + `espeak-ng-src/`) — the Dawn/WebGPU runtime DLLs (from the `onnxruntime-webgpu` wheel) + espeak-ng (x64 build + import lib + `espeak-ng-data`). |
...
| `kokoro-ocr/` | PP-OCR detection + recognition for the Cloud Reader path, a path dep of `kokoro-host` with a **target-neutral** API (no HTTP, pipe, Windows-UI or synthesis types): `session.rs` (the two ONNX sessions + the dictionary), `detect.rs` (DBNet post-processing), `recognize.rs` (line crop → CTC decode → words), `prep.rs`, `engine.rs` (the bounded worker), plus `examples/ocr-check.rs`. Its models are **not bundled** — the panel downloads them at first run. The engine argument lives in [`kokoro-ocr/README.md`](kokoro-ocr/README.md). |
| `kokoro-browser-extension/` | The Kindle Cloud Reader extension (bun/TypeScript, Chrome + Edge): captures the rendered page, posts it to the host's `/ocr`, narrates via `/synth`, and floats a word highlight over the page image. Reaches the host over loopback HTTP only, after a one-time pairing. See [`kokoro-browser-extension/README.md`](kokoro-browser-extension/README.md). |
