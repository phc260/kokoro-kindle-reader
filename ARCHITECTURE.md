...
| `THIRD_PARTY_NOTICES.md` + `licenses/` | Bundle licensing: the app's own code is MIT except for the files ported from `kokoro-js` and PaddleOCR (Apache-2.0, attributed there file by file), but the shipped binaries link espeak-ng (GPL-3.0-or-later, **modified** by `build-espeak.ps1`) and Slint-under-GPL, so a release is conveyed under GPLv3. `build-installer.ps1` stages both into the installer — they must ship *with* the binaries, not just live here. |
