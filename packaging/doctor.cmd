...
if not defined GIT (
    call :fail "Git" "install from https://git-scm.com/download/win"
) else if not defined LFS (
    call :fail "Git LFS" "install Git LFS, then: git lfs install, git lfs pull"
) else call :ok "Git" "git !GIT!, !LFS!"
...
if defined MSVC_OK (call :ok "MSVC" "!VS!") else call :fail "MSVC" "install VS Build Tools with 'Desktop development with C++'"
...
if defined CMAKE (call :ok "CMake" "!CMAKE!") else call :fail "CMake" "winget install Kitware.CMake"
...
if defined PY (call :ok "Python" "!PY!") else call :fail "Python 3" "install from https://www.python.org/downloads/ (tick 'Add to PATH')"

set "CARGO="
for /f "tokens=2" %%v in ('cargo --version 2^>nul') do set "CARGO=%%v"
if defined CARGO (call :ok "Rust" "cargo !CARGO!") else call :fail "Rust" "install from https://rustup.rs"

rustup target list --installed 2>nul | findstr /b /c:"i686-pc-windows-msvc" >nul
if not errorlevel 1 (call :ok "Rust x86 target" "i686-pc-windows-msvc") else call :fail "Rust x86 target" "rustup target add i686-pc-windows-msvc"
...
if defined RUSTSRC (call :ok "rust-src" "installed") else call :fail "rust-src" "rustup component add rust-src"

set "NSIS="
set "MAKENSIS=%PF86%\NSIS\makensis.exe"
if exist "%MAKENSIS%" for /f %%v in ('call "%MAKENSIS%" /VERSION 2^>nul') do set "NSIS=%%v"
if "!NSIS!"=="v3.12" (call :ok "NSIS" "v3.12") else call :fail "NSIS 3.12" "choco install nsis --version=3.12.0 -y --force"
...
if not defined ABOUT (
    call :fail "cargo-about" "!ABOUT_FIX!"
) else (
    ...
    if defined ABOUT_OK (call :ok "cargo-about" "!ABOUT!") else call :fail "cargo-about !ABOUT!, need 0.9.0 or later" "!ABOUT_FIX!"
)
...
if defined SEVENZIP (call :ok "7-Zip" "!SEVENZIP!") else call :warn "7-Zip" "choco install 7zip -y (only needed to verify a built installer)"

set "BUN="
for /f %%v in ('bun --version 2^>nul') do set "BUN=%%v"
if defined BUN (call :ok "bun" "!BUN!") else call :fail "bun" "install from https://bun.sh"
...
rem :ok/:fail/:warn NAME TEXT - print one line. The text goes through
