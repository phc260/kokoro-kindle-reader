...
| `doctor.ps1` | flutter-doctor style report of whether the TOOLS are installed - Python, Rust and the x86 target, git/git-lfs, NSIS, cargo-about, 7-Zip, bun - every missing one in a single pass. Shell, not Python, because it must run on a machine with no Python, the one prerequisite a Python script can never report. It does not inspect the provisioned state; `build_installer.py`'s preflight owns those pins. Windows only. |
| `doctor.cmd` | Runs `doctor.ps1` with `-ExecutionPolicy Bypass` for that one process, since a fresh Windows (policy `Restricted`) refuses every `.ps1` - and a fresh machine is what the doctor is for. Changes no policy; arguments and exit code pass through. This is the entry point to document. |
| `doctor.sh` | The Linux twin of `doctor.ps1`, same flutter-doctor shape and same shell-not-Python rationale. Covers only the two tiers Linux has - `app` (the host: synth core + loopback endpoint, no tray/panel/Kindle) and `extension`; there is no installer/source tier off Windows, so no NSIS/cargo-about/7-Zip/rust-src/x86-target checks. It adds a CMake + C-toolchain check because `fetch-deps.py` builds espeak from source, and gives distro-matched install commands (apt/dnf/pacman/zypper). Provisioned state is owned by `fetch-deps.py` + `kokoro-host`'s `build.rs`, not `build_installer.py`. |
...
tracked**. `fetch-deps.py` keeps it from the wheel into `native-deps\windows\runtime\notices\` and
