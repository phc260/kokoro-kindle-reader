...
| `about.toml`, `about.hbs` | `cargo-about`'s config (the accepted-licence list) and output template. |
...
- `LICENSE`, `THIRD_PARTY_NOTICES.md`, and `licenses\` — the checked-in texts (GPLv3,
  Apache-2.0, Unicode-3.0, BSD-2-Clause, BSD-3-Clause, NCSA), plus two generated
  subdirectories staged in at build time: `licenses\onnxruntime\` (that wheel's own
  notices) and `licenses\dependencies\` (the Rust closure's, one HTML file per shipped
  binary — see below)
