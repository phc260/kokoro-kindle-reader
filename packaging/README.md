...
| `generate-dependency-licenses.ps1` | Runs `cargo about` against each shipped crate's `Cargo.lock` and appends the exact licence/notice files from every resolved crate package that supplies them; called automatically by `build-installer.ps1`. Needs `cargo install cargo-about --locked --features cli` once. |
...
| `components.toml` | Checked-in inventory of every **non-Rust** component (shipped native DLLs, compiled-in SVGs, NSIS stub, plus the not-shipped downloaded assets — OCR + voice models — for attribution): origin, version/revision, SHA-256, SPDX, notice files, modification status. |
...
for the real notice in a crate's package.
...
*in the installer*, not just in `staging\`. Component notice paths come directly from
`components.toml`; an unparseable declaration, a directory path, or a component without
exactly one `notice` field also fails closed. On a tag, `build-corresponding-source.ps1`
