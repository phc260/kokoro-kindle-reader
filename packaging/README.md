...
| `doctor.cmd` | Developer check that lists which build tools are missing (Python, Rust + x86 target, git-lfs, MSVC, CMake, NSIS, cargo-about, 7-Zip, rust-src, bun): `doctor.cmd -For app\|installer\|source\|extension\|all`. Plain batch, so no execution policy applies. Tools only; provisioned state is `build_installer.py`'s job. |
| `doctor.sh` | Linux version: `app` (Python, Rust, git-lfs, C/C++ toolchain, CMake) and `extension` (bun). |
