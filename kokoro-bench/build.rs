// Link the prebuilt espeak-ng import lib (for the espeak.rs FFI, reused from
// kokoro-host via #[path]) and stage the Dawn ORT + espeak runtime DLLs +
// espeak-ng-data next to the exe. Mirrors kokoro-host/build.rs's staging step —
// this crate has no version resource/icon to embed, so that part is dropped.
...
fn main() {
    ...
    // kokoro-bench and native-deps are both direct children of the repo root.
    ...
    for p in [&espk_lib, &runtime, &espk_data] {
        if !p.exists() {
            panic!(
                "kokoro-bench: missing {} — run native-deps/fetch-deps.ps1 \
