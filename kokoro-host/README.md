...
| File | What |
|---|---|
| `main.rs` | `tao` event loop + `tray-icon` menu (Settings / Quit) + `auto-launch`. "Settings" spawns `kokoro-panel.exe`. `#![windows_subsystem = "windows"]` in release (no console). |
...
- **`controls.json` is the single source of truth, read live** (`%APPDATA%\com.phc260.kokoro-kindle-reader\`).
  The keys `read_controls` reads must match the keys `kokoro-panel` writes (`voice`,
  `speed`, `gain`, `chunk`, `kindle_kokoro`) — change them in both places.
